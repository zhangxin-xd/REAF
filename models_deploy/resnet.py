...
class conv_1(nn.Module):  
  ...
  def __init__(self, in_c, out_c):
    ...
    self.conv = nn.Conv2d(in_c, out_c, kernel_size=3, stride=1, padding=1, bias=True) 
  def forward(self, x):   
    x = self.conv(x)  
    return x 
...
class CifarResNet(nn.Module):
  ...
  def _make_layer(self, block, planes, blocks, stride=1):
    ...
    layers = []
    if stride ==1:
        layers.append(block(16, 16, stride, downsample = False))
    else:
        layers.append(block(planes//2, planes, stride, downsample = True))
