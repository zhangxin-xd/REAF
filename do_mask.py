...
class Mask:
    ...
    def get_filter_similar(self, weight_torch, distance_rate, length, dist_type="l2"):
        codebook = np.ones(length)
        if len(weight_torch.size()) == 4:
            filter_pruned_num = int(weight_torch.size()[0] * (1 - 0))
    ...
    def init_rate(self, args, epoch):
        ...
        for key in range(args.layer_begin, args.layer_end + 1, args.layer_inter):
            if args.af:
                self.compress_rate[key] = self.cal_rate(args,epoch) 
            else:
                self.compress_rate[key] = args.rate
        # different setting for  different architecture
        if args.arch == 'resnet20':
            last_index = 57
        elif args.arch == 'resnet32':
            last_index = 93
        elif args.arch == 'sres50':
            last_index = 207
        ...
        elif args.arch == 'src56':
            last_index = 330
            skip_list = [] # skip projection
            self.mask_shadow = [x for x in range(3, last_index, 6)] # 
    ...
    def init_mask(self, args,epoch):
        ...
        for index, item in enumerate(self.model.parameters()):
            if index in self.mask_index: # conv  resnet50  49 
                # mask for norm criterion
                self.mat[index], self.filter_index[index] =  self.get_filter_similar(item.data, self.compress_rate[index],
                                                           self.model_length[index])
                #self.get_filter_codebook(item.data, self.compress_rate[index],
                 #                                          self.model_length[index])
                #self.get_filter_similar(item.data, self.compress_rate[index],
                                                           #self.model_length[index])
                                                           #self.get_filter_codebook(item.data, self.compress_rate[index],
                                                           #self.model_length[index])
